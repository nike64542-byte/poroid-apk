...
class SettingsRepository @Inject constructor(
    ...
    companion object {
        /**
         * Absolute ceiling for terminal-window tabs. The UI never shows more
         * than this, and extra-tab socket files are swept on cleanup up to
         * this limit (not the configured count) to catch stale sockets from
         * a previously-higher setting. Guest hvc tty adapters must cover
         * this range — see the HVC_ALLOC_TTY_ADAPTERS patch in Dockerfile.
         */
        const val MAX_TERMINAL_TABS = 15
        const val MIN_TERMINAL_TABS = 1
        const val DEFAULT_TERMINAL_TABS = 8

        /** Multi-window terminal slots. See [DEFAULT_TERMINAL_TABS]. */
        val KEY_TERMINAL_TABS          = intPreferencesKey("terminal_tabs")
        ...
        /** First apt update done after fresh install. Auto-runs on first VM boot. */
        val KEY_FIRST_APT_UPDATE_DONE   = booleanPreferencesKey("first_apt_update_done")
    ...
    val terminalTabs: Flow<Int> = context.dataStore.data
        .catch { e -> if (e is IOException) emit(emptyPreferences()) else throw e }
        .map { (it[KEY_TERMINAL_TABS] ?: DEFAULT_TERMINAL_TABS).coerceIn(MIN_TERMINAL_TABS, MAX_TERMINAL_TABS) }
    ...
    suspend fun setTerminalTabs(value: Int)              = set(KEY_TERMINAL_TABS, value.coerceIn(MIN_TERMINAL_TABS, MAX_TERMINAL_TABS))
    ...
    suspend fun getTerminalTabsSnapshot()         = terminalTabs.first()
}
