...
/**
 * Multi-window terminal count. Mirrors VmCpuChips: a launch-time VM parameter, so
 * the caller gates it on the VM being stopped (changing it cannot affect a VM
 * that is already running — the virtio-console ports are wired at QEMU launch).
 */
@OptIn(ExperimentalLayoutApi::class)
@Composable
fun VmTerminalTabsChips(
    currentTabs: Int,
    onChange: (Int) -> Unit,
    enabled: Boolean = true,
    showDivider: Boolean = true,
    ...
    Column(modifier = Modifier.padding(bottom = PodroidTokens.Spacing.SM)) {
        Text(
            "${stringResource(R.string.terminal_windows)}  ·  $currentTabs",
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurface,
            modifier = Modifier.padding(
                top = PodroidTokens.Spacing.MD,
        ...
        FlowRow(
            ...
            val options = DeviceResourcePolicy.TERMINAL_TABS_OPTIONS.let { opts ->
                if (currentTabs in opts) opts else (opts + currentTabs).sorted()
            }
            options.forEach { n ->
                FilterChip(
                    selected = n == currentTabs,
                    enabled = enabled,
                    onClick = { onChange(n) },
                    label = {
                        Text(
                            "$n",
                            fontWeight = if (n == currentTabs) FontWeight.Bold else FontWeight.Normal,
                        )
                    },
                    shape = RoundedCornerShape(PodroidTokens.Radius.Chip),
                    colors = PodroidChipColors(),
        ...
        Text(
            text = stringResource(R.string.terminal_windows_hint),
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
            modifier = Modifier.padding(top = PodroidTokens.Spacing.SM),
        )
