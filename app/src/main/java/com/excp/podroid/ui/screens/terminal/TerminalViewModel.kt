...
import com.excp.podroid.engine.QemuEngine
...
class TerminalViewModel @Inject constructor(
    ...
    // ── Multi-window terminal tabs ────────────────────────────────────────────
    // Each tab maps to its own virtio-console pair in the VM (own getty/shell).
    // Only the active tab's TerminalView exists at a time, so the shared
    // sessionClient/viewClient below always target the active tab's session/ref;
    // switching tabs re-binds the session to a (new) view which replays the
    // session's retained transcript.
    private val _activeTab = kotlinx.coroutines.flow.MutableStateFlow(0)
    val activeTab: StateFlow<Int> = _activeTab

    private val _openTabs = kotlinx.coroutines.flow.MutableStateFlow(listOf(0))
    val openTabs: StateFlow<List<Int>> = _openTabs

    /** Multi-window tabs need extra virtio-console ports; only the QEMU backend
     *  wires those in. On AVF extra tabs would alias the single console, so the
     *  bar shows the primary tab only. */
    val supportsMultiTab: Boolean = engine.backendId == "qemu"

    /**
     * How many terminal windows this VM can run — the user-configured slot count.
     *
     * Safe to read off the setting rather than the live VM: Settings only lets
     * the value change while the VM is stopped (it is gated on vmNotRunning like
     * RAM/CPU) because the ports are wired at launch, so the setting always
     * describes the ports of the VM that is actually running.
     */
    val maxTerminalTabs: StateFlow<Int> = settingsRepository.terminalTabs
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), SettingsRepository.DEFAULT_TERMINAL_TABS)

    /** Sessions per tab, created lazily on demand. Tab 0's session also lives
     *  in the engine as the boot-time primary bridge. */
    private val tabSessions = HashMap<Int, TerminalSession>()

    /** Open the next free terminal tab and switch to it. Returns its index, or
     *  -1 when every slot is already open. */
    fun openTerminalTab(): Int {
        val open = _openTabs.value
        val next = (0 until maxTerminalTabs.value).firstOrNull { !open.contains(it) }
            ?: return -1
        _openTabs.value = open + next
        _activeTab.value = next
        return next
    }

    /** Close a terminal tab. Any tab can be closed (including the former
     *  "primary" tab 0) as long as at least one tab remains open - the last
     *  tab is protected so the terminal can never be emptied. Clearing a tab
     *  finishes its bridge session (guest getty respawns a fresh login on the
     *  port), so the slot can be re-opened cleanly. */
    fun closeTerminalTab(tab: Int) {
        if (_openTabs.value.size <= 1) return
        val open = _openTabs.value.filter { it != tab }
        _openTabs.value = open
        engine.closeTerminalSessionForTab(tab)
        tabSessions.remove(tab)
        if (_activeTab.value == tab) {
            _activeTab.value = open[0]
        }
    }

    /** Switch the active terminal tab. */
    fun selectTerminalTab(tab: Int) {
        if (tab !in _openTabs.value) return
    ...
    init {
        ...
        // Lowering the terminal-window setting retires the top slots, but a
        // surviving ViewModel can still hold tabs for them. Those windows have no
        // console behind them anymore, so close them instead of leaving terminals
        // that will never attach. The last tab is never dropped (matches
        // closeTerminalTab).
        viewModelScope.launch {
            maxTerminalTabs.collect { max ->
                val current = _openTabs.value
                val dropped = current.filter { it >= max }
                if (dropped.isEmpty()) return@collect
                dropped.forEach { tab ->
                    engine.closeTerminalSessionForTab(tab)
                    tabSessions.remove(tab)
                }
                val kept = current.filter { it < max }.ifEmpty { listOf(0) }
                _openTabs.value = kept
                if (_activeTab.value !in kept) _activeTab.value = kept[0]
            }
        }
    }
    ...
    val sessionClient = object : TerminalSessionClient {
        ...
        override fun onSessionFinished(finishedSession: TerminalSession) {
            ...
            // Only the ACTIVE tab's terminal is on screen; a background tab's
            // session finishing (its getty logged out, user closed it) must not
            // trigger a reconnect on the visible tab. The screen's reconnect
            // effect only watches the active session anyway.
    ...
    fun createSession() = createSessionForTab(_activeTab.value)

    /** Ensure the active tab's session exists (creating it via the engine) and
     *  point the shared clients at it. A no-op when it's already attached. */
    fun createSessionForTab(tab: Int) {
        if (attached && session != null && session == tabSessions[tab]) return

        val sess = tabSessions[tab]
        val created = if (sess != null && sess.isRunning) {
            sess
        } else {
            runCatching { engine.createTerminalSessionForTab(sessionClient, tab) }
                .onFailure { e ->
                    // AvfEngine throws UnsupportedOperationException until the
                    // AVF bridge is wired. Don't crash the UI — leave session null.
                    android.util.Log.w(TAG, "createTerminalSessionForTab(tab=$tab) failed on ${engine.backendId}: ${e.message}")
                }
                .getOrNull()?.also {
                    tabSessions[tab] = it
                    // Sanity: a non-primary tab must NEVER alias the primary
                    // session — that was the EngineHolder missing-override bug
                    // (all tabs shared one PTY, identical output everywhere).
                    val primary = tabSessions[0]
                    if (tab != 0 && it === primary) {
                        android.util.Log.e(TAG, "BUG: tab $tab session aliases primary session! Multi-tab is broken.")
                    }
                    android.util.Log.d(TAG, "tab $tab session=${System.identityHashCode(it)}")
                }
        }
        session = created
        attached = created != null
    }

    /** Session for a specific tab (created on demand). Shared with the engine's
     *  extra-session cache via createTerminalSessionForTab. */
    fun sessionForTab(tab: Int): TerminalSession? {
        val cached = tabSessions[tab]
        if (cached != null && cached.isRunning) return cached
        tabSessions.remove(tab)
        return null
    }

    /**
     * Force-drop the active tab's session. Called by the reconnect path to
     * replace a stale session from a previous run/attempt with a fresh one.
     */
    fun resetOnRestart() = resetOnRestartForTab(_activeTab.value)

    fun resetOnRestartForTab(tab: Int) {
        attached = false
        session = null
        tabSessions.remove(tab)
    }
    ...
    override fun onCleared() {
        ...
        attached = false
        // Drop our per-tab session references; the engine still owns its own
        // bridge sessions (primary + extras), so re-entering the terminal after
        // a config change re-attaches cleanly.
        session = null
        tabSessions.clear()
    }
