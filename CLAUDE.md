...
This repo is the **APK only** — it was split out of the former monorepo. The VM
...
Native binaries require **16KB page alignment** (`-Wl,-z,max-page-size=16384`) - mandatory on Android 13+; verified by an ELF parser in the poroid-qemu build.
...
> Repo layout note: this repo is the **APK only**. Kernel, initramfs/rootfs, and
> QEMU live in sibling repos (`poroid-kernel`, `poroid-rootfs`, `poroid-qemu`).
> Their artifacts are fetched at build time via `./fetch-artifacts.sh` (or CI
> downloads) from each repo's GitHub Release.
